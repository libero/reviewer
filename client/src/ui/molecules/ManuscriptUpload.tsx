...
const ManuscriptUpload: React.FC<Props> = ({ inactiveContent, activeContent }: Props): JSX.Element => {
    ...
    if (isDragActive) {
        content = <div className="manuscript-upload__content manuscript-upload__content--active">
            { activeContent ? activeContent : <p>{ t('ui:manuscript-upload.active-content') }</p>}
        </div>;
    } else {
        content = <div className="manuscript-upload__content manuscript-upload__content--inactive">
